...
class DBAccessor {
	...
	addToCart(userID, parameters, cb) {
	...
	getCartItemInfo(userID, parameters, cb) {
	...
	removeCartItem(userID, parameters, cb) {
	...
	addAddress(userID, parameters, cb) {
		if (parseInt(userID) > 0 && parameters != null) {
			let client = new pg.Client(config);
			client.connect((error) => {
				if (error) {
	...
	getAddressInfo(userID, parameters, cb) {
		if (parseInt(userID) > 0 && parameters != null && parseInt(parameters.index) >= 0) {
			let client = new pg.Client(config);
			client.connect((error) => {
				if (error) {
	...
	removeAddress(userID, parameters, cb) {
		if (parseInt(userID) > 0 && parameters != null && parameters.id != null) {
			let client = new pg.Client(config);
			client.connect((error) => {
				if (error) {
	...
	getUserOrderInfo(userID, parameters, cb) {
		if (parseInt(userID) > 0 && parameters != null && parseInt(parameters.index) >= 0) {
			let client = new pg.Client(config);
			client.connect((error) => {
				if (error) {
